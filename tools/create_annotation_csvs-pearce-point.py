...
def concat_annot(sel_tables_folder):
    ...
    files = glob.glob(sel_tables_folder + '\*.xlsx')
...
if __name__ == "__main__":
    ...
    output_dir = r'E:\baseline-with-normalization-reduce-tonal\fine-tuning'

    # path to folder with selection tables
    sel_table_path = r'E:\baseline-with-normalization-reduce-tonal\fine-tuning\CierrasFiles\formatted'

    # name of output selection tables excel workbook (just for reference)
    output_file_trim = output_dir + r'\pearce-point-false-cierra-annotations.xlsx'
    ...
    formatted_table = format_annot(all_annot_orig, output_name=output_dir + r'\pearce-point-false-cierra-annots.xlsx')
